...
//		c_log_s("f0");
		...
		can_get_rx_fifo_0_element(can_module, &rx_element_fifo_0, fifo0_getindex);
...
//		c_log_s("f1");
		...
		can_get_rx_fifo_1_element(can_module, &rx_element_fifo_1, fifo1_getindex);
...
//			xlog((uint8_t *) &buf_num,1);
		...
		if (checkbit(&CAN_flags, BUS_ON) && new_can_message) {
//			c_log('n');
			// check frame format
